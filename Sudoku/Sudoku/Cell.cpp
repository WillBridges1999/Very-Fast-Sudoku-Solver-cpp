...
{
	for (size_t index = 0; index < m_candidateList.size(); ++index)
	{
		if (m_candidateList[index] == value)
		{
			m_candidateList.erase(m_candidateList.begin() + index);
			break;
		}
	}
}
...
/*void Cell::removeCandidateValue(const int value)
{
	...
	{
		if (m_candidateList. == value)
		{
			m_candidateList.erase(m_candidateList.at[it + 1]);
		}
	}
}*/